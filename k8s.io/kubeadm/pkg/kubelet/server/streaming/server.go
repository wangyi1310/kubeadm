...
type Config struct {
	...
	// k8s.io/kubeadm/pkg/kubelet/server/remotecommand/constants.go for available protocols.
	// Only used for SPDY streaming.
	SupportedRemoteCommandProtocols []string

	// The streaming protocols the server supports (understands and permits).  See
	// k8s.io/kubeadm/pkg/kubelet/server/portforward/constants.go for available protocols.
	// Only used for SPDY streaming.
