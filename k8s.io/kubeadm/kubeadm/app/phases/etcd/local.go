...
func GetEtcdPodSpecHttp(cfg *kubeadmapi.InitConfiguration) v1.Pod {
	pathType := v1.HostPathDirectoryOrCreate
	etcdMounts := map[string]v1.Volume{
		etcdVolumeName: staticpodutil.NewVolume(etcdVolumeName, cfg.Etcd.Local.DataDir, &pathType),
	}
	return staticpodutil.ComponentPod(v1.Container{
		Name:            kubeadmconstants.Etcd,
		Command:         getEtcdCommandHttp(cfg),
		Image:           images.GetEtcdImage(cfg),
		ImagePullPolicy: v1.PullIfNotPresent,
		// Mount the etcd datadir path read-write so etcd can store data in a more persistent manner
		VolumeMounts: []v1.VolumeMount{
...
func getEtcdCommandHttp(cfg *kubeadmapi.InitConfiguration) []string {
	defaultArguments := map[string]string{
		...
		"initial-cluster":             fmt.Sprintf("%s=http://127.0.0.1:2380", cfg.GetNodeName()),
	}
	command := []string{"etcd"}
